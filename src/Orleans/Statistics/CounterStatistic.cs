...
namespace Orleans.Runtime
{
    ...
    internal class FixedSizedCounterStatistic : ICounter<long>
    {
        private class ReferenceFixedSizedLongs
        {
            internal FixedSizedQueue<long> Values = new FixedSizedQueue<long>(BUFFER_SIZE);
        }

        private const int BUFFER_SIZE = 100;

        [ThreadStatic]
        private static List<ReferenceFixedSizedLongs> allStatisticsFromSpecificThread;
        [ThreadStatic]
        private static bool isOrleansManagedThread;

        private static readonly Dictionary<string, FixedSizedCounterStatistic> registeredStatistics;
        private static readonly object lockable;
        private static int nextId;
        private readonly ConcurrentStack<ReferenceFixedSizedLongs> specificStatisticFromAllThreads;

        private readonly int id;
        private long last;
        private bool firstStatDisplay;
        private Func<long, long> valueConverter;
        private FixedSizedQueue<long> nonOrleansThreadsCounter; // one for all non-Orleans threads
        private readonly bool isHidden;
        private readonly string currentName;
        ...
        static FixedSizedCounterStatistic()
        {
            registeredStatistics = new Dictionary<string, FixedSizedCounterStatistic>();
            nextId = 0;
            lockable = new object();
        }

        // Must be called while Lockable is locked
        private FixedSizedCounterStatistic(string name, bool useDelta, CounterStorage storage, bool isHidden)
        {
            Name = name;
            currentName = Metric.CreateCurrentName(name);
            UseDelta = useDelta;
            ...
            firstStatDisplay = true;
            valueConverter = null;
            nonOrleansThreadsCounter = new FixedSizedQueue<long>(BUFFER_SIZE);
            this.isHidden = isHidden;
            specificStatisticFromAllThreads = new ConcurrentStack<ReferenceFixedSizedLongs>();
        }
        ...
        internal static void SetOrleansManagedThread()
        {
            if (!isOrleansManagedThread)
            {
                ...
                {
                    isOrleansManagedThread = true;
                    allStatisticsFromSpecificThread = new List<ReferenceFixedSizedLongs>(new ReferenceFixedSizedLongs[BUFFER_SIZE]);
                }
            }
        }

        public static FixedSizedCounterStatistic FindOrCreate(StatisticName name, bool useDelta, CounterStorage storage, bool isHidden = false)
        {
            return FindOrCreate_Impl(name, useDelta, storage, isHidden);
        }

        private static FixedSizedCounterStatistic FindOrCreate_Impl(StatisticName name, bool useDelta, CounterStorage storage, bool isHidden)
        {
            lock (lockable)
            {
                FixedSizedCounterStatistic stat;
                if (registeredStatistics.TryGetValue(name.Name, out stat))
                {
                    return stat;
                }
                var ctr = new FixedSizedCounterStatistic(name.Name, useDelta, storage, isHidden);
        ...
        public FixedSizedCounterStatistic AddValueConverter(Func<long, long> converter)
        {
            this.valueConverter = converter;
            return this;
        }
        ...
        public void IncrementBy(long n)
        {
            if (isOrleansManagedThread)
            {
                if (allStatisticsFromSpecificThread.Count <= id)
                {
                    ...
                    allStatisticsFromSpecificThread.AddRange(new ReferenceFixedSizedLongs[bufferSize + 1]);
                }
                ...
                if (orleansValue == null)
                {
                    orleansValue = new ReferenceFixedSizedLongs();
                    allStatisticsFromSpecificThread[id] = orleansValue;
                    specificStatisticFromAllThreads.Push(orleansValue);
                }

                orleansValue.Values.Enqueue(n);
            }
            else
            {
                /*
                ...
                lock (lockable)
                {
                    nonOrleansThreadsCounter.Enqueue(n);
                }
            }
        }
        ...
        public static void AddCounters(List<ICounter> list, Func<FixedSizedCounterStatistic, bool> predicate)
        {
            lock (lockable)
            {
                list.AddRange(registeredStatistics.Values.Where(c => !c.isHidden && predicate(c)));
